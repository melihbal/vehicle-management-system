...
import { UserFilter } from '../../user-components/user-filter/user-filter';
...
@Component({
  ...
  imports: [
    ...
    DxDataGridModule, DxDropDownBoxModule, DxTreeViewModule,
    DxButtonModule, DxTextBoxModule, DxDateBoxModule, DxDateRangeBoxModule, DxListModule,
    UserTable, UserFilter, VehicleCreate, VehicleDelete, VehicleEdit, VehicleReport
  ],
  templateUrl: './users.html',
  styleUrls: ['./users.css']
})
export class Users implements OnInit {
  protected readonly title = signal('UsersSystem');
  ...
  showWarningNoSuchUser: boolean = false;

  warningMessageCreate: string = 'Warning: The ID and Lisence Plate values do not match';
  warningMessageDelete: string = 'Warning: le matching the properties found';
  warningMessageEdit: string = 'Warning: le matching the properties found';
  ...
  loadUsers() {
    ...
    // }
    this.http.get<UserDisplay[]>(`${this.apiUrl}/GetUsers`)
      .subscribe({
        next: (data) => {
          this.users = data;
          console.log('users: ', this.users)
  
          this.uniqueIds = Array.from(new Set(this.users.map(v => v.id))).sort((a, b) => a - b);
  ...
  loadSpecificUser(searchInput: string): void {
    console.log(searchInput);
    this.http.get<UserDisplay>(`${this.apiUrl}/GetUser?username=${searchInput}`)
    .subscribe({
      next: (data) => {
        this.users = [data];
        console.log(data);
        this.showWarningNoSuchUser = false;
      },
      error: err => {
        console.error(err);
        this.showWarningNoSuchUser = true;
        console.log(this.showWarningNoSuchUser);
      }
    });
  }

  usersUpdatedMain(user: string): void {
    console.log("Received:", user);
    this.loadSpecificUser(user);
  }

  clearClicked(str: string) {
    if(str == '') {
      console.log("hello");
      this.loadUsers();
      this.showWarningNoSuchUser = false;
